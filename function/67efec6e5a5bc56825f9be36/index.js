...
export async function charge(payment_bucket_data, payment) {
  ...
  await Bucket.data.update(`${PAYMENT_BUCKET_ID}`, payment_bucket_data._id, payment_bucket_data).then((bucket_data) => {
...
export async function subscribe(payment_bucket_data, payment) {
  ...
  payment.payment_method ? (information.default_payment_method = payment.payment_method.payment_method_id) : null;
